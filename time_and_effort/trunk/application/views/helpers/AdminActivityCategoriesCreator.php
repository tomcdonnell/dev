...
class AdminActivityCategoriesCreator
{
    ...
    private function getAsArray()
    {
        return array(
            ...
            'By Completion Date'   => self::_createDatesCategories(
    ...
    private function _createApprovalStatusCategories()
    {
        ...
        $alphabetSplitCategoryChildren = $this->_getAlphabetSplitCategoryChildren
        (
        ...
        foreach ($branchNamesByDivisionName as $divisionName => $branchNames) {
            ...
            foreach ($branchNames as $branchName) {
                $branchCategoryInfoByName[$branchName] = array(
                    ...
                    'children'          => $this->_getAlphabetSplitCategoryChildren(
                        'Employee First Name', 'staff.first_name', array(
                            'name'         => 'STAFF_FIRST_NAME_BEGINNING_WITH_LETTER',
     ...
     */
    private function _getAlphabetSplitCategoryChildren(
        $splitFieldDisplayName, $sqlColumnName, $subSubCategoryInfo = null
    )
    {
        ...
        for ($ascii = ord('A'); $ascii <= ord('Z'); ++$ascii) {
            ...
            $subSubCategoryInfo['letterUcase'] = $cUpper;
            $category = array(
                'sqlJoinClause'     => '',
                'children'          => (
                    ($subSubCategoryName === null)? array():
                    $this->_getSpecialCategoryChildren($subSubCategoryInfo)
                ),
                'sqlWhereCondition' => (
     ...
     *
     */
    private function _getStaffNamesBeginningWithLetterCategoryChildren(
        $sqlJoinClauses, $specialCategoryInfo
    )
    {
        return
    }
     ...
     *
     */
    private function _getSpecialCategoryChildren($specialCategoryInfo)
    {
        if (!array_key_exists('name', $specialCategoryInfo)) {
            throw new Exception('Expected array key does not exist.');
        }

        switch ($specialCategoryInfo['name']) {
            case 'employeeName':
                $children = $this->_getStaffNamesBeginningWithLetterCategoryChildren(
                    array(
            ...
            case 'managerName':
                $children = $this->_getStaffNamesBeginningWithLetterCategoryChildren(
                    array('JOIN staff ON (activity.idStaff_leadTeamManager=staff.id)'),
                    $specialCategoryInfo
                );
                break;
            default:
                throw new Exception("Unknown special category name '$specialCategoryName'.");
        }
     ...
     *
     */
    private function _getStaffNamesBeginnningWithLetterCategoryChildren(
        $sqlJoinClauses, $categoryInfo
    )
    {
        $this->db->query(
            'SELECT identifier, CONCAT(first_name, " ", last_name) AS name_full
             FROM staff
            ' . implode("\n", $sqlJoinClauses) . '
             WHERE division=?
             AND branch=?
             AND first_name LIKE "?%',
            $categoryInfo['divisionName'],
            $categoryInfo['branchName'  ],
            $categoryInfo['letterUcase' ]
        ) 
    }
