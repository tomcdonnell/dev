 ...
 *
 */
function ExplorableTable(table, ajaxUrl)
{
   var f = 'ExplorableTable()';
   UTILS.checkArgs(f, arguments, [HTMLTableElement, String]);
    ...
    *
    */
   this.getCategoryRowsViaAjax = function (idCategory)
   {
      var f = 'ExplorableTable.getCategoryRowsViaAjax()';
      UTILS.checkArgs(f, arguments, [Number]);

      _domElements.tbody.innerHTML = '';
      _appendLoadingCategoryRowsRowAfterSavedRow();
      $.ajax({data: {action: 'getChildCategoriesInfo', idCategory: idCategory}});
   };
    ...
    *
    */
   function _receiveAjaxMessage(msg, textStatus, xmlHttpRequest)
   {
      try
      {
         var f = 'ExplorableTable._receiveAjaxMessage()';
         UTILS.checkArgs(f, arguments, [Object, String, XMLHttpRequest]);
         ...
         switch (msg.action)
         {
          case 'getColumnHeadings':
            _state.columnHeadings = msg.reply; _initThead();
            break;
      ...
      try
      {
         ...
         switch (command)
         {
          case 'expand_dataRows':
            ...
            $.ajax({data: {action: 'getDataRowsForCategory', idCategory: idCategory}});
            break;
   ...
   function _initThead()
   {
      var f = 'ExplorableTable._initThead()';
      UTILS.checkArgs(f, arguments, []);

      _domElements.thead.innerHtml               = '';
      _guiElements.variableWidthTableHeadingsRow = new InteractiveTableHeadingsRow
      (
         _state.columnHeadings
      );

      _state.trToAppendCategoryRowsAfter = null;
      $.ajax({
         data: {action: 'getChildCategoriesInfo', idCategory: _ID_CATEGORY_BY_APPROVAL_STATUS}
      });
   }
   ...
   function _init()
   {
      ...
      $.ajax({data: {action: 'getColumnHeadings'}});
   }
   ...
   function _createCategoryRowsFromCategoriesInfo(categoriesInfo)
   {
      ...
      for (var i = 0; i < categoriesInfo.length; ++i)
      {
         ...
         td.appendChild(document.createTextNode(info.name + ' (' + info.n_childDataRows + ')'));
   ...
   function _appendDataRowsAfterSavedRow(trs)
   {
      ...
      var headingsTrs = $(_guiElements.variableWidthTableHeadingsRow.getTr()).clone(true);
   ...
   function _convertArraysOfStringsToTrsWithSeparators(arraysOfStrings)
   {
      ...
      for (var i = 0; i < arraysOfStrings.length; ++i)
      {
         ...
         trs.push(UTILS.DOM.implode(separatorTh, tds, TR(), true));
      }
   ...
   {
      table: table,
      thead: $(table).children()[0], // NOTE: Must be no whitespace between <table>  and <thead>.
      tbody: $(table).children()[1]  // NOTE: Must be no whitespace between </thead> and <tbody>.
   };

   var _guiElements =
   {
      variableWidthTableHeadingsRow: null // To be created in _initThead().
   };
