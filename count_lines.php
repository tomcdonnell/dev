	...
	**/
	...
	class Folder {
		...
		function Folder($path) {
			...
			$this -> folders 	= array();
			$this -> files		= array();
			$this -> exclude_extensions = array('gif', 'jpg', 'jpeg', 'png', 'tft', 'bmp', 'log', 'rest-of-the-file-extensions-to-exclude');
		...
		function count_lines() {
			...
			foreach($this -> folders as $folder) {
				if( in_array($folder -> name, $this -> exclude_folders) || substr($folder->name, 0, 1) == '.') {
